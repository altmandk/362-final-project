...
*/
  ...
  for (x = -7; x <= 7; x += 1) {
    y = 3*x;
    Trace(tft, x, y, 1, 60, 290, 390, 260, -7, 7, 1, -7, 7, 1, "Sin(x)", "x", "fn(x)", update1, YELLOW);
  }
/*
  delay(2000);
...
double x, y;
  ...
  //TODO figure out how to get equation from other arduino using serial connection
  //then pass the equation in as x in the trace function

  //TODO figure out how to parse equation into string to set it as title of the graph

  
  update1 = true;
  for (x = -7; x <= 7; x += 1) {
    y = sin(x);
    Trace(tft, x, y, 1, 60, 290, 390, 260, -7, 7, 1, -7, 7, 1, "", "x", "y", update1, YELLOW);
  }
  delay(2000);
